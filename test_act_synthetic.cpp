...
int main() {
    try {
        ...
        size_t best_dict_std = 0, best_dict_cpu = 0; int best_order_std = 0, best_order_cpu = 0;
        double best_time_ms_std = 0.0, best_time_ms_cpu = 0.0;
        ...
        for (size_t idx = 0; idx < sweep.size(); ++idx) {
            ...
            // Build dictionaries explicitly (ACT standard)
            ...
            // Build dictionaries explicitly (ACT_CPU Eigen+CBLAS)
            ...
            auto result_cpu = act_cpu.transform(signal, cfg.order);
            auto t2b = std::chrono::steady_clock::now();

            double dict_elapsed_ms_std = std::chrono::duration<double, std::milli>(t1_std - t0_std).count();
            double trans_elapsed_ms_std = std::chrono::duration<double, std::milli>(t2a - t1a).count();
            double dict_elapsed_ms_cpu = std::chrono::duration<double, std::milli>(t1_cpu - t0_cpu).count();
            double trans_elapsed_ms_cpu = std::chrono::duration<double, std::milli>(t2b - t1b).count();

            if (result_std.params.empty() || result_cpu.params.rows() == 0) {
                std::cerr << "FAIL: No chirplets found" << std::endl;
                return 1;
            }

            // Compute output SNR vs ground truth for both
            ...
            double output_snr_db_std = compute_snr_std(result_std.approx);
            double improvement_db_std = output_snr_db_std - input_snr_db;

            double output_snr_db_cpu = compute_snr_cpu(result_cpu.approx);
            double improvement_db_cpu = output_snr_db_cpu - input_snr_db;

            size_t dict_size_std = act_std.get_dict_size();
            size_t dict_size_cpu = act_cpu.get_dict_size();
            double est_bytes = static_cast<double>(dict_size_std) * length * sizeof(double);
            double est_mb = est_bytes / (1024.0 * 1024.0);

            std::cout << "ACT (std) dict size:  " << dict_size_std << ", est matrix memory: ~" << est_mb << " MB\n";
            ...
            std::cout << "ACT_CPU (Eigen+BLAS) dict size: " << dict_size_cpu << ", est matrix memory: ~" << est_mb << " MB\n";
            ...
            for (size_t r = 0; r < kmax_std; ++r) {
                ...
                if (best_idx >= 0) {
                    ...
                    double coeff = (r < result_std.coeffs.size() ? result_std.coeffs[r] : 0.0);
