...
int main(int argc, char** argv) {
    print_separator("ACT TK (custom kernel) PROFILING TEST");
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if ((a == "--len" || a == "-l") && i + 1 < argc) SIGNAL_LENGTH = std::atoi(argv[++i]);
            ...
            std::cout << "Usage: " << (argv && argv[0] ? argv[0] : "profile_tk")
    ...
    timer.start();
    ACT_CUDA_TK_f act(FS, SIGNAL_LENGTH, ranges, true);
