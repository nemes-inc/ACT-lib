...
mcmcreport::mcmcreport() : _mcmcreport_owner() ,nfev(p_struct->nfev),acceptrate(p_struct->acceptrate),autocorrtimes(&p_struct->autocorrtimes)
{
}

mcmcreport::mcmcreport(alglib_impl::mcmcreport *attach_to):_mcmcreport_owner(attach_to) ,nfev(p_struct->nfev),acceptrate(p_struct->acceptrate),autocorrtimes(&p_struct->autocorrtimes)
{
}

mcmcreport::mcmcreport(const mcmcreport &rhs):_mcmcreport_owner(rhs) ,nfev(p_struct->nfev),acceptrate(p_struct->acceptrate),autocorrtimes(&p_struct->autocorrtimes)
{
}
     ...
     ae_state *_state)
{
    ...
    ae_assert(popsize>=2, "MCMCSetAlgoGoodmanWeare: PopSize<2", _state);
    ae_assert(epochscnt>=1, "MCMCSetAlgoGoodmanWeare: EpochsCnt<1", _state);
...
ae_bool mcmciteration(mcmcstate* state, ae_state *_state)
{
    ...
    if( state->rstate.stage>=0 )
    {
        ...
        groupsize = state->rstate.ia.ptr.p_int[7];
        i0 = state->rstate.ia.ptr.p_int[8];
        i1 = state->rstate.ia.ptr.p_int[9];
        j0 = state->rstate.ia.ptr.p_int[10];
        j1 = state->rstate.ia.ptr.p_int[11];
        src = state->rstate.ia.ptr.p_int[12];
        dst = state->rstate.ia.ptr.p_int[13];
        acceptcnt = state->rstate.ia.ptr.p_int[14];
    ...
    {
        iallocv(state->popsize, &state->propidx, _state);
        rallocv(state->popsize, &state->propz, _state);
        rallocm(state->popsize, n+1, &state->propxf, _state);
        iallocv(state->popsize, &state->grpidx, _state);
    }
    else
    {
        iallocv(1, &state->propidx, _state);
        rallocv(1, &state->propz, _state);
        rallocm(1, n+1, &state->propxf, _state);
    }
    ...
    if( state->useparallelmoves )
    {
        ...
        {
            state->grpidx.ptr.p_int[i] = i;
        }
        ae_assert(state->popsize>=2, "MCMC: integrity check 294008 failed", _state);
        state->grp0size = state->popsize/2;
    }
    if( state->x0type==0 )
    {
        ae_assert(state->cntx0>=1, "MCMC: integrity check 426148 failed", _state);
        rallocm(state->popsize, n+1, &state->population, _state);
        for(i=0; i<=state->popsize-1; i++)
        {
            for(j=0; j<=n-1; j++)
            {
                state->population.ptr.pp_double[i][j] = state->x0m.ptr.pp_double[0][j]+state->x0stddev*state->s.ptr.p_double[j]*hqrndnormal(&state->globalrs, _state);
            }
        }
    }
    if( state->x0type==1 )
    {
        ae_assert(state->cntx0>=1, "MCMC: integrity check 487508 failed", _state);
        k = ae_minint(state->cntx0, state->popsize, _state);
        rallocm(state->popsize, n+1, &state->population, _state);
        for(i=0; i<=k-1; i++)
        {
            for(j=0; j<=n-1; j++)
            {
                state->population.ptr.pp_double[i][j] = state->x0m.ptr.pp_double[i][j];
            }
        }
        if( k<state->popsize )
        {
            ...
            for(i=k; i<=state->popsize-1; i++)
            {
                for(j=0; j<=n-1; j++)
                {
                    state->population.ptr.pp_double[i][j] = hqrndnormal(&state->globalrs, _state)*(0.33*state->tmp2.ptr.p_double[j]*state->s.ptr.p_double[j])+state->tmp0.ptr.p_double[j];
                }
            }
        }
    }
    if( state->x0type==2 )
    {
         ...
         */
        k = ae_minint(state->lastpopulationsize, state->popsize, _state);
        rallocm(state->popsize, n+1, &state->population, _state);
        for(i=0; i<=k-1; i++)
        {
            for(j=0; j<=n-1; j++)
            {
                state->population.ptr.pp_double[i][j] = state->lastpopulation.ptr.pp_double[i][j];
            }
        }
        for(i=k; i<=state->popsize-1; i++)
        {
            for(j=0; j<=n-1; j++)
            {
                state->population.ptr.pp_double[i][j] = state->lastpopulation.ptr.pp_double[hqrnduniformi(&state->globalrs, state->lastpopulationsize, _state)][j];
            }
        }
        ...
        {
            icopyv(state->popsize, &state->lastgrpidx, &state->grpidx, _state);
        }
    }
    i = 0;
lbl_4:
    ...
    for(j=0; j<=n-1; j++)
    {
        state->querydata.ptr.p_double[j] = state->population.ptr.pp_double[i][j];
    }
...
lbl_0:
    ...
    state->population.ptr.pp_double[i][n] = state->replyfi.ptr.p_double[0];
    i = i+1;
    goto lbl_4;
lbl_6:
    if( !state->xrep )
...
lbl_7:
    ...
    ae_assert(state->popsize>=2, "MCMC: integrity check 238039 failed", _state);
    itmax = icase2(state->initialstart, state->burninlen, 0, _state)+state->epochscnt*state->thinby;
    rallocm(state->epochscnt*state->popsize, n+1, &state->repsample, _state);
    itidx = 0;
lbl_9:
    if( itidx>itmax-1 )
    {
     ...
     */
    acceptcnt = 0;
    groupscnt = icase2(state->useparallelmoves, 2, state->popsize, _state);
    i = 0;
lbl_12:
    if( i>groupscnt-1 )
    {
    ...
    if( state->useparallelmoves )
    {
        ...
        {
            dst = state->grpidx.ptr.p_int[i0+j];
            src = state->grpidx.ptr.p_int[j0+hqrnduniformi(&state->globalrs, j1-j0, _state)];
            state->propidx.ptr.p_int[j] = dst;
            v = ae_sqr((mcmc_goodmanwearea-(double)1)*hqrnduniformr(&state->globalrs, _state)+(double)1, _state)/mcmc_goodmanwearea;
            state->propz.ptr.p_double[j] = v;
            for(k=0; k<=n-1; k++)
            {
                state->propxf.ptr.pp_double[j][k] = state->population.ptr.pp_double[src][k]+v*(state->population.ptr.pp_double[dst][k]-state->population.ptr.pp_double[src][k]);
            }
        }
    }
    else
    {
        groupsize = 1;
        j = hqrnduniformi(&state->globalrs, state->popsize-1, _state);
        if( j>=i )
        {
            j = j+1;
        }
        state->propidx.ptr.p_int[0] = i;
        v = ae_sqr((mcmc_goodmanwearea-(double)1)*hqrnduniformr(&state->globalrs, _state)+(double)1, _state)/mcmc_goodmanwearea;
        state->propz.ptr.p_double[0] = v;
        for(k=0; k<=n-1; k++)
        {
            state->propxf.ptr.pp_double[0][k] = state->population.ptr.pp_double[j][k]+v*(state->population.ptr.pp_double[i][k]-state->population.ptr.pp_double[j][k]);
        }
    }
...
lbl_17:
    ...
    for(j=0; j<=groupsize-1; j++)
    {
        if( ae_fp_less(hqrnduniformr(&state->globalrs, _state),ae_minreal(1.0, ae_exp((double)(n-1)*ae_log(state->propz.ptr.p_double[j], _state)+state->propxf.ptr.pp_double[j][n]-state->population.ptr.pp_double[state->propidx.ptr.p_int[j]][n], _state), _state)) )
        {
            rcopyrr(n+1, &state->propxf, j, &state->population, state->propidx.ptr.p_int[j], _state);
...
lbl_14:
    ...
    if( state->useparallelmoves )
    {
        for(i=0; i<=state->popsize-1; i++)
        {
            j = i+hqrnduniformi(&state->globalrs, state->popsize-i, _state);
            k = state->grpidx.ptr.p_int[i];
            state->grpidx.ptr.p_int[i] = state->grpidx.ptr.p_int[j];
            state->grpidx.ptr.p_int[j] = k;
        }
    }
    ...
    if( state->dotrace )
    {
        mcmc_dologging(state, itidx, acceptcnt, _state);
    }
    ...
    ae_assert(state->repsample.rows>=state->repsamplesize+state->popsize&&state->repsample.cols>=n+1, "MCMC: integrity check 497055 failed", _state);
    for(i=0; i<=state->popsize-1; i++)
    {
        rcopyrr(n+1, &state->population, i, &state->repsample, state->repsamplesize, _state);
...
lbl_rcomm:
    ...
    state->rstate.ia.ptr.p_int[7] = groupsize;
    state->rstate.ia.ptr.p_int[8] = i0;
    state->rstate.ia.ptr.p_int[9] = i1;
    state->rstate.ia.ptr.p_int[10] = j0;
    state->rstate.ia.ptr.p_int[11] = j1;
    state->rstate.ia.ptr.p_int[12] = src;
    state->rstate.ia.ptr.p_int[13] = dst;
    state->rstate.ia.ptr.p_int[14] = acceptcnt;
     ...
     ae_state *_state)
{
    ...
    *samplesize = 0;
    ...
    rep->acceptrate = (double)state->repacceptcnt/coalesce((double)(state->repepochscnt*state->reppopsize), (double)(1), _state);
...
ae_bool mcmcrestart(mcmcstate* state, ae_state *_state)
{
    ...
    ae_vector_set_length(&state->rstate.ia, 14+1, _state);
     ...
     ae_state *_state)
{
    ...
    hqrndseed(state->rngseed, 856446, &state->globalrs, _state);
    state->reseedglobalrs = ae_true;
    state->useparallelmoves = ae_false;
    ae_vector_set_length(&state->rstate.ia, 14+1, _state);
    ae_vector_set_length(&state->rstate.ra, 0+1, _state);
    state->rstate.stage = -1;
}
     ...
     ae_state *_state)
{
    ...
    if( state->dodetailedtrace )
    {
        ...
        for(i=0; i<=popsize-1; i++)
        {
            for(j=0; j<=n-1; j++)
            {
                popmean.ptr.p_double[j] = popmean.ptr.p_double[j]+state->population.ptr.pp_double[i][j];
            }
        }
        for(j=0; j<=n-1; j++)
        {
            popmean.ptr.p_double[j] = popmean.ptr.p_double[j]/(double)popsize;
        }
        for(i=0; i<=popsize-1; i++)
        {
            for(j=0; j<=n-1; j++)
            {
                popstddev.ptr.p_double[j] = popstddev.ptr.p_double[j]+(state->population.ptr.pp_double[i][j]-popmean.ptr.p_double[j])*(state->population.ptr.pp_double[i][j]-popmean.ptr.p_double[j]);
            }
        }
        for(j=0; j<=n-1; j++)
        {
            popstddev.ptr.p_double[j] = ae_sqrt(popstddev.ptr.p_double[j]/(double)popsize, _state);
        }
        ae_trace("pop.mean   = ");
        ...
        for(i=0; i<=popsize-1; i++)
        {
            llmean = llmean+state->population.ptr.pp_double[i][n];
        }
        llmean = llmean/(double)popsize;
        llstddev = (double)(0);
        for(i=0; i<=popsize-1; i++)
        {
            llstddev = llstddev+ae_sqr(state->population.ptr.pp_double[i][n]-llmean, _state);
        }
        llstddev = ae_sqrt(llstddev/(double)popsize, _state);
        ae_trace("loglik.mean   = %0.3f\n",
            (double)(llmean));
        ae_trace("loglik.stddev = %0.3f\n",
            (double)(llstddev));
        ae_trace("accept.rate   = %0.3f\n",
            (double)((double)acceptcnt/(double)popsize));
    }
    else
    {
    }
...
static void mcmc_savepopulation(mcmcstate* state, ae_state *_state)
{
    ...
    rcopyallocm(state->popsize, state->n+1, &state->population, &state->lastpopulation, _state);
    if( state->useparallelmoves )
    {
        icopyallocv(state->popsize, &state->grpidx, &state->lastgrpidx, _state);
    }
}
     ...
     ae_state *_state)
{
    ...
    ae_assert(epochscnt*popsize==state->repsamplesize, "MCMC: integrity check 657126 failed", _state);
    ...
    for(idxv=0; idxv<=n-1; idxv++)
    {
        ...
        for(idxw=0; idxw<=popsize-1; idxw++)
        {
            ...
            raddv(epochscnt, (double)1/(double)popsize, &a2, &a, _state);
        }
        rmulv(epochscnt, (double)1/a.ptr.p_double[0], &a, _state);
     ...
     ae_state *_state)
{
    ...
    epochscnt = state->repepochscnt;
    popsize = state->reppopsize;
    ae_assert(epochscnt*popsize==state->repsamplesize, "MCMC: integrity check 678132 failed", _state);
    ex2 = 1;
    while(ex2<epochscnt)
    {
        ex2 = ex2*2;
    ...
    {
        x.ptr.p_double[i] = state->repsample.ptr.pp_double[state->reppopsize*i+idxw][idxv];
...
void _mcmcstate_init(void* _p, ae_state *_state, ae_bool make_automatic)
{
    ...
    ae_matrix_init(&p->population, 0, 0, DT_REAL, _state, make_automatic);
    ...
    ae_vector_init(&p->grpidx, 0, DT_INT, _state, make_automatic);
    ae_matrix_init(&p->lastpopulation, 0, 0, DT_REAL, _state, make_automatic);
    ae_vector_init(&p->lastgrpidx, 0, DT_INT, _state, make_automatic);
...
void _mcmcstate_init_copy(void* _dst, const void* _src, ae_state *_state, ae_bool make_automatic)
{
    ...
    dst->xrep = src->xrep;
    ae_matrix_init_copy(&dst->population, &src->population, _state, make_automatic);
    ...
    ae_vector_init_copy(&dst->grpidx, &src->grpidx, _state, make_automatic);
    dst->grp0size = src->grp0size;
    dst->haslastpopulation = src->haslastpopulation;
    ae_matrix_init_copy(&dst->lastpopulation, &src->lastpopulation, _state, make_automatic);
    ae_vector_init_copy(&dst->lastgrpidx, &src->lastgrpidx, _state, make_automatic);
    dst->lastpopulationsize = src->lastpopulationsize;
    ...
    dst->repacceptcnt = src->repacceptcnt;
    dst->repepochscnt = src->repepochscnt;
    dst->reppopsize = src->reppopsize;
...
void _mcmcstate_clear(void* _p)
{
    ...
    ae_vector_clear(&p->s);
    ae_matrix_clear(&p->population);
    _hqrndstate_clear(&p->globalrs);
    ae_vector_clear(&p->propidx);
    ae_vector_clear(&p->propz);
    ae_matrix_clear(&p->propxf);
    ae_vector_clear(&p->grpidx);
    ae_matrix_clear(&p->lastpopulation);
    ae_vector_clear(&p->lastgrpidx);
...
void _mcmcstate_destroy(void* _p)
{
    ...
    ae_matrix_destroy(&p->population);
    _hqrndstate_destroy(&p->globalrs);
    ae_vector_destroy(&p->propidx);
    ae_vector_destroy(&p->propz);
    ae_matrix_destroy(&p->propxf);
    ae_vector_destroy(&p->grpidx);
    ae_matrix_destroy(&p->lastpopulation);
    ae_vector_destroy(&p->lastgrpidx);
