// Synthetic ACT multithreaded test (per-signal parallelism) using SIMD implementation
// Generates a batch of identical signals and runs ACT_SIMD_MultiThreaded over them.
...
int main() {
    try {
        ...
        for (size_t idx = 0; idx < sweep.size(); ++idx) {
            ...
            std::cout << "\n=== MT SIMD Config " << (idx+1) << "/" << sweep.size() << " ===\n";
            ...
            ACT_SIMD_MultiThreaded act(fs, length, "synth_dict_cache_simd_mt.bin", ranges, false, true, false);
            auto t1 = std::chrono::steady_clock::now();
            auto results = act.transform_batch_simd_parallel(signals, cfg.order, false, 0);
            ...
            if (pass) {
                std::cout << "\nSynthetic ACT SIMD MT test passed at this configuration." << std::endl;
                if (noiseless) {
