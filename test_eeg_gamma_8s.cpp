...
std::vector<double> load_eeg_data(const std::string& filename, int max_samples = 2048) {
...
// Create optimized parameter ranges for 8-second analysis
ACT::ParameterRanges create_8s_parameter_ranges(int signal_length) {
    std::cout << "\n🎯 8-SECOND EEG GAMMA ANALYSIS PARAMETERS:" << std::endl;
    std::cout << "────────────────────────────────────────────────────────────────" << std::endl;
    
    // High temporal resolution for 8s analysis
    double tc_step = 4.0;  // ~15.6ms resolution
    std::cout << "⏱️  Time Center (tc): 0 to " << signal_length-1 << " samples, step " << tc_step << std::endl;
    
    // Focused gamma band
    double fc_min = 25.0, fc_max = 49.0, fc_step = 2.0;
    std::cout << "🌊 Frequency Center (fc): " << fc_min << " to " << fc_max << " Hz, step " << fc_step << std::endl;
    
    // Extended duration range for diversity
    double logDt_min = -3.5, logDt_max = -0.5, logDt_step = 0.3;
    ...
    // Moderate chirp rate range
    double c_min = -15.0, c_max = 15.0, c_step = 3.0;
...
int main() {
    ...
    const int MAX_SAMPLES = 2048;  // 8 seconds
    
    print_separator("8-SECOND EEG GAMMA BAND ACT ANALYSIS");
    
    // Load EEG data
    std::cout << "Loading EEG data (8 seconds)...\n";
    ...
    auto ranges = create_8s_parameter_ranges(samples.size());
    
    // Initialize ACT with SIMD optimization
    std::cout << "\n🚀 Initializing ACT with SIMD optimization..." << std::endl;
    ACT_SIMD act_simd(FS, samples.size(), "eeg_gamma_8s_dict.bin", ranges, 
    ...
    // Save results to CSV
    save_results_to_csv(result, FS, "eeg_gamma_results_8s.csv", "8s_analysis");
    
    std::cout << "\n🎉 8-second EEG gamma analysis complete!" << std::endl;
    std::cout << "📁 Results saved to eeg_gamma_results_8s.csv" << std::endl;
    std::cout << "🎨 Visualize with: python ../Adaptive_Chirplet_Transform/visualize_eeg_gamma.py --results original_8s" << std::endl;
